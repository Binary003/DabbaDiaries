...
export function mapCook(row: Record<string, unknown>): CookProfile {
  return {
    ...
    pricePerMeal: Number(row.price_per_meal ?? 0), weeklyPrice: Number(row.weekly_price ?? 0), monthlyPrice: Number(row.monthly_price ?? 0), capacity: Number(row.daily_capacity ?? 1), capacityCap: 20, selfDelivery: Boolean(row.self_delivery_enabled), selfDeliveryFee: Number(row.self_delivery_fee ?? 0), fssaiTier: 'basic-registration', fssaiNumber: String(row.fssai_number ?? ''), verificationStatus: row.status === 'active' ? 'approved' : 'pending', status: row.status as CookProfile['status'], zoneId: row.zone_id ? String(row.zone_id) : undefined, latitude: row.latitude == null ? undefined : Number(row.latitude), longitude: row.longitude == null ? undefined : Number(row.longitude), mealsPerDay: 1, activeSubscribers: Number(row.active_subscribers ?? 0),
  };
}
